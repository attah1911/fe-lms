...
import { FiArrowLeft, FiPlus, FiDownload, FiCalendar, FiClipboard, FiEye, FiRefreshCw, FiBookOpen, FiUsers } from "react-icons/fi";
...
export type MataPelajaranDetailRole = "admin" | "guru";

interface PropTypes {
  role: MataPelajaranDetailRole;
}

const MataPelajaranDetail: React.FC<PropTypes> = ({ role }) => {
  ...
  useEffect(() => {
    if (activeTab === 'tugas' && assignments.length === 0) {
      fetchAssignments();
    }
  ...
  }, [activeTab, id, assignments.length, enrolledStudents.length, fetchAssignments, fetchEnrolledStudents]);
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                  <div className="w-full">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2">
                      <div className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded flex items-center">
                        <FiBookOpen size={12} className="mr-1" />
                        {assignment.materi?.judul || "Materi tidak diketahui"}
                      </div>
                  ...
                  <div className="flex sm:flex-shrink-0 mt-2 sm:mt-0 w-full sm:w-auto">
                    <Button 
                      ...
                      startContent={<FiEye size={14} />}
                      onPress={() => {
                        router.push(`/${role}/matapelajaran/${id}/tugas/${assignment._id}`);
                      }}
                      className="w-full sm:w-auto"
                    >
                      Lihat Detail
