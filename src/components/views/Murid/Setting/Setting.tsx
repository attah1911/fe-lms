...
import { FiUser, FiMail, FiLock, FiEdit2, FiSave, FiUpload, FiPhone, FiCalendar, FiBookOpen } from "react-icons/fi";
...
import { Student, StudentUpdateData, kelasList } from "../../../../types/Student";
...
const Setting: React.FC = () => {
  ...
  const [studentUpdateData, setStudentUpdateData] = useState<StudentUpdateData>({
    nis: "",
    kelas: "",
    noTelp: "",
  });
  ...
  const fetchProfileAndStudentData = async () => {
    try {
      setLoading(true);
      ...
      // Fetch student data - wrap in try/catch so it doesn't affect the whole component if it fails
      try {
        const studentResponse = await studentServices.getStudentProfile();
  ...
  const handleStudentDataUpdate = async () => {
    try {
      ...
      // Validate inputs
      if (!studentUpdateData.nis || !studentUpdateData.kelas) {
        toast.error('Mohon isi NIS dan kelas');
        setSaving(false);
        return;
      }
      
      try {
        // This should be replaced with actual student data update endpoint
        const response = await studentServices.updateStudentProfile(studentUpdateData);
        ...
        console.error("Error updating student data:", updateErr);
        toast.error(updateErr.response?.data?.message || updateErr.message || 'Gagal memperbarui data murid');
      }
    } catch (err: any) {
      console.error("General error:", err);
    ...
    <PageContainer>
      ...
      {loading ? (
        ...
        <Tabs aria-label="Settings options" className="mt-4">
          ...
          <Tab key="student-data" title="Data Murid">
            <Card className="mt-5">
              <CardHeader className="border-b border-divider">
                <div className="flex flex-col">
                  <h2 className="text-lg font-semibold">Informasi Murid</h2>
                  <p className="text-small text-default-500">Kelola data murid Anda</p>
                </div>
              </CardHeader>
              <CardBody>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Input
                    ...
                    value={studentUpdateData.nis}
                    onChange={(e) => setStudentUpdateData({...studentUpdateData, nis: e.target.value})}
                  ...
                  <Select
                    ...
                    selectedKeys={studentUpdateData.kelas ? [studentUpdateData.kelas] : []}
                    onChange={(e) => setStudentUpdateData({...studentUpdateData, kelas: e.target.value})}
                    startContent={<FiBookOpen className="text-default-400" />}
                    isRequired
                  >
                    {kelasList.map((kelas) => (
                  ...
                  <Input
                    ...
                    value={studentUpdateData.noTelp}
                    onChange={(e) => setStudentUpdateData({...studentUpdateData, noTelp: e.target.value})}
                ...
                <div className="flex justify-end mt-4">
                  <Button
                    color="primary"
                    startContent={<FiSave />}
                    onClick={handleStudentDataUpdate}
                    isLoading={saving}
                  >
                    Simpan Data Murid
                  </Button>
